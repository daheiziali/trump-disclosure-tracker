...
> 实现优先级：`PRODUCT_METRICS_AND_DISPLAY_SPEC.md` 是当前统计口径和状态逻辑的唯一验收依据。本文件描述仓库与原型现状；原型中的旧标签或旧计算不得覆盖该规范。
...
- 基于最新有效 OGE 278e 年报披露建立基准快照
- 纳入基准日之后已去重的 OGE 278-T 交易披露作为增量流水
- 用官方金额区间中点做披露口径估算
...
PRODUCT_METRICS_AND_DISPLAY_SPEC.md  # 当前统计口径与状态逻辑验收规范
FUTURE_ARCHITECTURE_NOTES.md      # 远期架构设想，不作为当前统计逻辑依据
...
1. 交易排行榜：持仓规模、高频交易、买入规模、卖出规模
2. 最新交易披露

其中：

- 四类排行榜的“查看更多”进入统一完整列表，并继承来源模块的默认排序和时间口径
- `最新披露交易` 的“查看更多”进入完整交易记录表
- `资产详情` 默认选中推算规模最高资产，并展示交易披露链

当前 `frontend/index.html` 和 `demo/index.html` 仍属于历史原型/视觉演示，其内嵌统计结果不代表新规则已经完成后端实现。技术验收必须重新对照 `PRODUCT_METRICS_AND_DISPLAY_SPEC.md`。
