...
六个状态必须互斥。同一资产、同一统计时点只能得到一个最终状态，后端不得输出状态数组，也不得输出用「或」「+」「/」拼接的组合状态，例如：
...
```text
current_status      单值枚举：新进/增持/减持/疑似清仓/疑似新进/仍持有
last_period_action  最近一次可判断交易动作：买入/卖出/neutral_same_day
```

`current_status` 表示规则计算后的唯一持仓状态；`last_period_action` 表示后台用于判断状态的最近交易动作，两者是不同维度。持仓列表前端显示 `current_status` 状态标签和 `last_period_trade_date` 最近交易日期，但不额外显示「买入/卖出」动作文字；交易动作只在资产交易链等交易明细中展示。旧接口中的 `inferred_status` 如继续保留，也必须遵守与 `current_status` 相同的单值枚举约束。

动作与状态必须满足以下兼容关系：

```text
last_period_action = 买入：current_status 只能是 新进/增持/疑似新进
last_period_action = 卖出：current_status 只能是 减持/疑似清仓
近一年无买入或卖出：current_status 才能是 仍持有
```

因此「仍持有」不能与近一年内的最近买入或卖出同时成立。若接口返回互相冲突的值，应拒绝发布该条聚合结果并进入后台复核，不能由前端同时展示两个结论。
...
状态引擎必须使用 `if / else if` 或等价的单分支决策结构，并在落库前校验 `current_status` 仅命中一个枚举值。状态筛选只读取 `current_status`，不得再根据买入/卖出动作追加第二个状态。
...
- 除「全部」外，六个状态互斥；每个资产只能命中一个状态筛选项。
- 「全部」只表示不过滤状态，不作为数据字段值存储。
- 持仓列表显示唯一持仓状态标签和最近交易日期，不显示「买入/卖出」动作文字；交易动作仅在交易链等明细页面展示。
- 禁止出现「买入 + 仍持有」「卖出 + 仍持有」或任何组合状态文案。
- 用户端不出现「状态不明」。
- 主列表金额字段统一为「披露规模估算」。
...
- 可按模块输出独立投资主题占比。
- 可按最终状态规则输出资产状态。
- `current_status` 为单值枚举，且与 `last_period_action` 通过动作兼容矩阵校验。
- 任一资产同时命中多个状态、输出组合状态或出现动作冲突时，聚合任务必须报错并进入后台复核。
